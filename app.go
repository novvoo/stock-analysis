...
func (a *App) GetStockData() (string, error) {
	...
	// Calculate the actual end date (excluding weekends)
	endDate := now
	for endDate.Weekday() == time.Saturday || endDate.Weekday() == time.Sunday {
		endDate = endDate.AddDate(0, 0, -1)
	}

	// Calculate 180 days ago from the adjusted end date
	...
	for i, item := range hqData {
		...
		case []interface{}:
			if len(row) >= 9 {
				...
				turnoverRate := 0.0
				if i >= 5 {
					prevItem := results[i-5]
					if prevItem.Volume != 0 {
						volumeRate = (volume - prevItem.Volume) / prevItem.Volume * 100
					}
					if prevItem.Turnover != 0 {
						turnoverRate = (turnover - prevItem.Turnover) / prevItem.Turnover * 100
					}
				}
